...
export class ElasticHandlebarsVisitor extends Handlebars.Visitor {
  ...
  PathExpression(path: hbs.AST.PathExpression) {
    const blockParamId =
      ...
      this.blockParamIndex(path.parts[0]);
  ...
  private processStatementOrExpression(
    node: ProcessableNodeWithPathPartsOrLiteral
  ) {
  ...
  private blockParamIndex(name: string): [number, any] | undefined {
    for (
      let depth = 0, len = this.blockParamNames.length;
      depth < len;
      depth++
    ) {
  ...
  private blockValue(node: hbs.AST.BlockStatement, value: any) {
    ...
    const result = this.container.hooks.blockHelperMissing!.call(
      this.context,
      value,
      options
    );
  ...
  private processHelperNode(node: ProcessableNodeWithPathParts) {
    ...
    if (
      this.compileOptions.knownHelpers &&
      this.compileOptions.knownHelpers[name]
    ) {
  ...
  private invokePartial(
    ...
    const options: AmbiguousHelperOptions & ResolvePartialOptions =
      this.setupParams(partial, name);
  ...
  private processAmbiguousNode(node: ProcessableNodeWithPathParts) {
    ...
    if (isBlock(node)) {
      const result = helper.fn
        ? helperResult
        : this.container.hooks.blockHelperMissing!.call(
            this.context,
            helperResult,
            helper.options
          );
  ...
  private setupParams(
    node: ProcessableBlockStatementNode,
    name: string
  ): HelperOptions;
  private setupParams(
    node: ProcessableStatementNode,
    name: string
  ): NonBlockHelperOptions;
  private setupParams(
    node: ProcessableNode,
    ...
    if (isBlock(node)) {
      (options as HelperOptions).fn = node.program
        ? this.processDecorators(
            node.program,
            this.generateProgramFunction(node.program)
          )
        : noop;
      (options as HelperOptions).inverse = node.inverse
        ? this.processDecorators(
            node.inverse,
            this.generateProgramFunction(node.inverse)
          )
        : noop;
    }
  ...
  private generateProgramFunction(program: hbs.AST.Program) {
    ...
    const prog: TemplateDelegate = (
      ...
      runtimeOptions.partials =
        runtimeOptions.partials || this.runtimeOptions!.partials;
